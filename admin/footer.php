      ...
      datasets: [{
...

        {
          ...
          data: [
            ...
            for ($bulan = 1; $bulan <= 12; $bulan++) {
              $thn_ini = date('Y');
              $cicilan = mysqli_query($koneksi, "select sum(transaksi_nominal) as total_cicilan from transaksi where transaksi_jenis='cicilan' and month(transaksi_tanggal)='$bulan' and year(transaksi_tanggal)='$thn_ini'");
              $ci = mysqli_fetch_assoc($cicilan);

              // $total = str_replace(",", "44", number_format($ci['total_cicilan']));
              $total = $ci['total_cicilan'];
              if ($ci['total_cicilan'] == "") {
                echo "0,";
              } else {
      ...
      datasets: [{
...

        {
          ...
          data: [
            ...
            while ($t = mysqli_fetch_array($tahun)) {
              $thn = $t['tahun'];
              $cicilan = mysqli_query($koneksi, "select sum(transaksi_nominal) as total_cicilan from transaksi where transaksi_jenis='Cicilan' and year(transaksi_tanggal)='$thn'");
              $ci = mysqli_fetch_assoc($cicilan);
              $total = $ci['total_cicilan'];
              if ($ci['total_cicilan'] == "") {
                echo "0,";
